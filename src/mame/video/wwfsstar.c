...
static TILE_GET_INFO( get_fg0_tile_info )
{
	...
	**- End of Comments -*/
	UINT16 *tilebase =  &state->fg0_videoram[tile_index * 2];
	INT32 tileno =  (tilebase[1] & 0x00ff) | ((tilebase[0] & 0x000f) << 8);
	INT32 colbank = (tilebase[0] & 0x00f0) >> 4;

	SET_TILE_INFO( 0, tileno, colbank, 0 );
}
...
static TILE_GET_INFO( get_bg0_tile_info )
{
	...
	**- End of Comments -*/
	UINT16 *tilebase =  &state->bg0_videoram[tile_index * 2];
	INT32 tileno = (tilebase[1] & 0x00ff) | ((tilebase[0] & 0x000f) << 8);
	INT32 colbank = (tilebase[0] & 0x0070) >> 4;
	INT32 flipx = (tilebase[0] & 0x0080) >> 7;

	SET_TILE_INFO( 2, tileno, colbank, flipx ? TILE_FLIPX : 0 );
}
...
static void draw_sprites(running_machine *machine, bitmap_t *bitmap, const rectangle *cliprect )
{
	...
	**- End of Comments - */
	...
	UINT16 *finish = source + 0x03ff / 2;
	int xpos, ypos, colourbank, flipx, flipy, chain, number, count;

	for ( ; source < finish; source += 5)
	{
		if (source [1] & 0x0001)
		{
