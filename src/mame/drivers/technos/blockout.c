...
static INPUT_PORTS_START( blockout )
	...
	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_BUTTON4 ) PORT_NAME("P1 Drop Button")
	...
	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_BUTTON4 ) PORT_PLAYER(2) PORT_NAME("P2 Drop Button")
...
static INPUT_PORTS_START( agress )
	...
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_BUTTON3 ) PORT_PLAYER(1) PORT_NAME("P1 Button3 (Bomb)")

	PORT_MODIFY("P2")
	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_BUTTON1 ) PORT_PLAYER(2)
	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_BUTTON2 ) PORT_PLAYER(2)
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_BUTTON3 ) PORT_PLAYER(2) PORT_NAME("P2 Button3 (Bomb)")
...
static MACHINE_RESET( blockout )
{
	...
	for (int i = 0; i < 0x4000 / 2; i++)
		state->frontvideoram[i] = state->frontvideoram[0x4000 / 2 + i] = i;
}
...
static MACHINE_DRIVER_START( blockout )
	...
	MDRV_CPU_ADD("maincpu", M68000, XTAL_10MHz)       /* MRH - 8.76 makes gfx/adpcm samples sync better -- but 10 is correct speed*/
	MDRV_CPU_PROGRAM_MAP(main_map)
	MDRV_TIMER_ADD_SCANLINE("scantimer", blockout_scanline, "screen", 0, 1)
	...
	MDRV_SCREEN_RAW_PARAMS(28000000 / 4, 448, 0, 320, 272, 10, 250)
