...
INLINE UINT32 get_texel_rgb15_bilinear(const render_texinfo *texture, INT32 curu, INT32 curv)
{
	...
	rgb_t filtered = rgb_bilinear_filter(pix00, pix01, pix10, pix11, curu >> 8, curv >> 8);
...
{
	PIXEL_TYPE *dest = (PIXEL_TYPE *)dstdata + y * pitch + x;

	UINT32 dpix = NO_DEST_READ ? 0 : *dest;
	register UINT32 dr = SOURCE32_R(col) + DEST_R(dpix);
	register UINT32 dg = SOURCE32_G(col) + DEST_G(dpix);
	register UINT32 db = SOURCE32_B(col) + DEST_B(dpix);
...
static void FUNC_PREFIX(draw_line)(const render_primitive *prim, void *dstdata, INT32 width, INT32 height, UINT32 pitch)
{
	...
	float alphavar = 255.0f * prim->color.a;
	UINT32 col = MAKE_RGB((int)(prim->color.r * alphavar), (int)(prim->color.g * alphavar), (int)(prim->color.b * alphavar));
	...
	/* use good old Bresenham for non-antialiasing 980317 BW */
	else
	{
		x1 = (x1 + 0x8000) >> 16;
		y1 = (y1 + 0x8000) >> 16;
...
static void FUNC_PREFIX(draw_rect)(const render_primitive *prim, void *dstdata, INT32 width, INT32 height, UINT32 pitch)
{
	...
	if (PRIMFLAG_GET_BLENDMODE(prim->flags) == BLENDMODE_NONE || IS_OPAQUE(prim->color.a))
	{
		...
		UINT32 pix = DEST_RGB_TO_PIXEL(r, g, b);
	...
	else if (!IS_TRANSPARENT(prim->color.a))
	{
		...
		float alphavar = 256.0f * prim->color.a;
		UINT32 r = (UINT32)(prim->color.r * alphavar);
		UINT32 g = (UINT32)(prim->color.g * alphavar);
		UINT32 b = (UINT32)(prim->color.b * alphavar);
		UINT32 inva = (UINT32)(256.0f - alphavar);
		...
		for (INT32 y = starty; y < endy; y++)
		{
			...
			for (INT32 x = startx; x < endx; x++)
			{
				...
				dr = (r + ((dpix & rmask) * inva)) & (rmask << 8);
				dg = (g + ((dpix & gmask) * inva)) & (gmask << 8);
				db = (b + ((dpix & bmask) * inva)) & (bmask << 8);
				*dest++ = (dr | dg | db) >> 8;
			}
		}
	}
...
static void FUNC_PREFIX(draw_quad_palette16_none)(const render_primitive *prim, void *dstdata, UINT32 pitch, quad_setup_data *setup)
{
	...
	if (IS_OPAQUE(prim->color.a))
	{
		/* fast case: no coloring, no alpha */
		if (prim->color.r >= 1.0f && prim->color.g >= 1.0f && prim->color.b >= 1.0f)
		{
			INT32 deviation;
			/* loop over rows */
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				...
				deviation = y - setup->starty;
				curu = setup->startu + setup->dudy * deviation;
				curv = setup->startv + setup->dvdy * deviation;

				/* loop over cols */
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(palette16)(&prim->texture, curu, curv);
		...
		else
		{
			...
			/* clamp R,G,B to 0-256 range */
			if (sr > 0x100)
				sr = ((INT32)sr < 0) ? 0 : 0x100;
			if (sg > 0x100)
				sg = ((INT32)sg < 0) ? 0 : 0x100;
			if (sb > 0x100)
				sb = ((INT32)sb < 0) ? 0 : 0x100;

			/* loop over rows */
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				PIXEL_TYPE *dest = (PIXEL_TYPE *)dstdata + y * pitch + setup->startx;
				curu = setup->startu + (y - setup->starty) * setup->dudy;
				curv = setup->startv + (y - setup->starty) * setup->dvdy;

				/* loop over cols */
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(palette16)(&prim->texture, curu, curv);
					r = (SOURCE32_R(pix) * sr) >> 8;
					g = (SOURCE32_G(pix) * sg) >> 8;
					b = (SOURCE32_B(pix) * sb) >> 8;
	...
	{
		float alphavar = 256.0f * prim->color.a;
		UINT32 sr = (UINT32)(prim->color.r * alphavar);
		UINT32 sg = (UINT32)(prim->color.g * alphavar);
		UINT32 sb = (UINT32)(prim->color.b * alphavar);
		UINT32 invsa = (UINT32)(256.0f - alphavar);
		UINT32 dpix;
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			for (INT32 x = setup->startx; x < endx; x++)
			{
				pix = GET_TEXEL(palette16)(&prim->texture, curu, curv);
				dpix = NO_DEST_READ ? 0 : *dest;
				r = (SOURCE32_R(pix) * sr + DEST_R(dpix) * invsa) >> 8;
				g = (SOURCE32_G(pix) * sg + DEST_G(dpix) * invsa) >> 8;
				b = (SOURCE32_B(pix) * sb + DEST_B(dpix) * invsa) >> 8;
	...
	if (prim->color.r >= 1.0f && prim->color.g >= 1.0f && prim->color.b >= 1.0f && IS_OPAQUE(prim->color.a))
	{
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			for (INT32 x = setup->startx; x < endx; x++)
			{
				...
				{
					dpix = NO_DEST_READ ? 0 : *dest;
					r = SOURCE32_R(pix) + DEST_R(dpix);
					g = SOURCE32_G(pix) + DEST_G(dpix);
					b = SOURCE32_B(pix) + DEST_B(dpix);
	...
	else
	{
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			for (INT32 x = setup->startx; x < endx; x++)
			{
				...
				{
					dpix = NO_DEST_READ ? 0 : *dest;
					r = ((SOURCE32_R(pix) * sr) >> 8) + DEST_R(dpix);
					g = ((SOURCE32_G(pix) * sg) >> 8) + DEST_G(dpix);
					b = ((SOURCE32_B(pix) * sb) >> 8) + DEST_B(dpix);
					r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
					g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
					b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
					*dest++ = DEST_ASSEMBLE_RGB(r, g, b);
...
static void FUNC_PREFIX(draw_quad_palettea16_alpha)(const render_primitive *prim, void *dstdata, UINT32 pitch, quad_setup_data *setup)
{
	...
	if (prim->color.r >= 1.0f && prim->color.g >= 1.0f && prim->color.b >= 1.0f && IS_OPAQUE(prim->color.a))
	{
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			for (INT32 x = setup->startx; x < endx; x++)
			{
				pix = GET_TEXEL(palette16a)(&prim->texture, curu, curv);
				ta = pix >> 24;
				if (ta != 0)
				{
					UINT32 dpix = NO_DEST_READ ? 0 : *dest;
					UINT32 invta = 0x100 - ta;
					r = (SOURCE32_R(pix) * ta + DEST_R(dpix) * invta) >> 8;
					g = (SOURCE32_G(pix) * ta + DEST_G(dpix) * invta) >> 8;
					b = (SOURCE32_B(pix) * ta + DEST_B(dpix) * invta) >> 8;
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			for (INT32 x = setup->startx; x < endx; x++)
			{
				...
				if (ta != 0)
				{
					...
					r = (SOURCE32_R(pix) * sr * ta + DEST_R(dpix) * invsta) >> 24;
					g = (SOURCE32_G(pix) * sg * ta + DEST_G(dpix) * invsta) >> 24;
					b = (SOURCE32_B(pix) * sb * ta + DEST_B(dpix) * invsta) >> 24;
	...
	if (IS_OPAQUE(prim->color.a))
	{
		...
		if (prim->color.r >= 1.0f && prim->color.g >= 1.0f && prim->color.b >= 1.0f)
		{
			...
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = ycc_to_rgb(GET_TEXEL(yuy16)(&prim->texture, curu, curv));
					*dest++ = SOURCE32_TO_DEST(pix);
					curu += dudx;
					curv += dvdx;
				}
		...
		if (prim->color.r >= 1.0f && prim->color.g >= 1.0f && prim->color.b >= 1.0f)
		{
			...
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				...
				if (palbase == NULL)
				{
					...
					for (INT32 x = setup->startx; x < endx; x++)
					{
						pix = GET_TEXEL(rgb15)(&prim->texture, curu, curv);
				...
				else
				{
					...
					{
						pix = GET_TEXEL(rgb15)(&prim->texture, curu, curv);
						r = palbase[(pix >> 10) & 0x1f] >> SRCSHIFT_R;
						g = palbase[(pix >> 5) & 0x1f] >> SRCSHIFT_G;
						b = palbase[(pix >> 0) & 0x1f] >> SRCSHIFT_B;
		...
		else
		{
			...
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				...
				if (palbase == NULL)
				{
					...
					{
						pix = GET_TEXEL(rgb15)(&prim->texture, curu, curv);
						r = (SOURCE15_R(pix) * sr) >> 8;
						g = (SOURCE15_G(pix) * sg) >> 8;
						b = (SOURCE15_B(pix) * sb) >> 8;
				...
				else
				{
					...
					{
						pix = GET_TEXEL(rgb15)(&prim->texture, curu, curv);
						r = (palbase[(pix >> 10) & 0x1f] * sr) >> (8 + SRCSHIFT_R);
						g = (palbase[(pix >> 5) & 0x1f] * sg) >> (8 + SRCSHIFT_G);
						b = (palbase[(pix >> 0) & 0x1f] * sb) >> (8 + SRCSHIFT_B);
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			if (palbase == NULL)
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(rgb15)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = (SOURCE15_R(pix) * sr + DEST_R(dpix) * invsa) >> 8;
					g = (SOURCE15_G(pix) * sg + DEST_G(dpix) * invsa) >> 8;
					b = (SOURCE15_B(pix) * sb + DEST_B(dpix) * invsa) >> 8;
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(rgb15)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = ((palbase[(pix >> 10) & 0x1f] >> SRCSHIFT_R) * sr + DEST_R(dpix) * invsa) >> 8;
					g = ((palbase[(pix >> 5) & 0x1f] >> SRCSHIFT_G) * sg + DEST_G(dpix) * invsa) >> 8;
					b = ((palbase[(pix >> 0) & 0x1f] >> SRCSHIFT_B) * sb + DEST_B(dpix) * invsa) >> 8;
		...
		if (prim->color.r >= 1.0f && prim->color.g >= 1.0f && prim->color.b >= 1.0f)
		{
			...
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				...
				if (palbase == NULL)
				{
					...
					for (INT32 x = setup->startx; x < endx; x++)
					{
						pix = GET_TEXEL(rgb32)(&prim->texture, curu, curv);
						*dest++ = SOURCE32_TO_DEST(pix);
						curu += dudx;
						curv += dvdx;
					}
				...
				else
				{
					...
					{
						pix = GET_TEXEL(rgb32)(&prim->texture, curu, curv);
						r = palbase[(pix >> 16) & 0xff] >> SRCSHIFT_R;
						g = palbase[(pix >> 8) & 0xff] >> SRCSHIFT_G;
						b = palbase[(pix >> 0) & 0xff] >> SRCSHIFT_B;
		...
		else
		{
			...
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				...
				else
				{
					...
					{
						pix = GET_TEXEL(rgb32)(&prim->texture, curu, curv);
						r = (palbase[(pix >> 16) & 0xff] * sr) >> (8 + SRCSHIFT_R);
						g = (palbase[(pix >> 8) & 0xff] * sg) >> (8 + SRCSHIFT_G);
						b = (palbase[(pix >> 0) & 0xff] * sb) >> (8 + SRCSHIFT_B);
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(rgb32)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = ((palbase[(pix >> 16) & 0xff] >> SRCSHIFT_R) * sr + DEST_R(dpix) * invsa) >> 8;
					g = ((palbase[(pix >> 8) & 0xff] >> SRCSHIFT_G) * sg + DEST_G(dpix) * invsa) >> 8;
					b = ((palbase[(pix >> 0) & 0xff] >> SRCSHIFT_B) * sb + DEST_B(dpix) * invsa) >> 8;
		...
		if (prim->color.r >= 1.0f && prim->color.g >= 1.0f && prim->color.b >= 1.0f)
		{
			...
			for (INT32 y = setup->starty; y < setup->endy; y++)
			{
				...
				if (palbase == NULL)
				{
					...
					for (INT32 x = setup->startx; x < endx; x++)
					{
						pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
						dpix = NO_DEST_READ ? 0 : *dest;
						r = SOURCE32_R(pix) + DEST_R(dpix);
						g = SOURCE32_G(pix) + DEST_G(dpix);
						b = SOURCE32_B(pix) + DEST_B(dpix);
				...
				else
				{
					...
					for (INT32 x = setup->startx; x < endx; x++)
					{
						pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
						dpix = NO_DEST_READ ? 0 : *dest;
						r = (palbase[(pix >> 16) & 0xff] >> SRCSHIFT_R) + DEST_R(dpix);
						g = (palbase[(pix >> 8) & 0xff] >> SRCSHIFT_G) + DEST_G(dpix);
						b = (palbase[(pix >> 0) & 0xff] >> SRCSHIFT_B) + DEST_B(dpix);
						r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
						g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
						b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
						*dest++ = DEST_ASSEMBLE_RGB(r, g, b);
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			if (palbase == NULL)
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = ((SOURCE32_R(pix) * sr * sa) >> 16) + DEST_R(dpix);
					g = ((SOURCE32_G(pix) * sg * sa) >> 16) + DEST_G(dpix);
					b = ((SOURCE32_B(pix) * sb * sa) >> 16) + DEST_B(dpix);
					r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
					g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
					b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
					*dest++ = DEST_ASSEMBLE_RGB(r, g, b);
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = ((palbase[(pix >> 16) & 0xff] * sr * sa) >> (16 + SRCSHIFT_R)) + DEST_R(dpix);
					g = ((palbase[(pix >> 8) & 0xff] * sr * sa) >> (16 + SRCSHIFT_R)) + DEST_G(dpix);
					b = ((palbase[(pix >> 0) & 0xff] * sr * sa) >> (16 + SRCSHIFT_R)) + DEST_B(dpix);
					r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
					g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
					b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
					*dest++ = DEST_ASSEMBLE_RGB(r, g, b);
...
static void FUNC_PREFIX(draw_quad_argb32_alpha)(const render_primitive *prim, void *dstdata, UINT32 pitch, quad_setup_data *setup)
{
	...
	INT32 endx = setup->endx;
	INT32 curu, curv;
	UINT32 r, g, b, pix, dpix, ta, invta, invsta;
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					...
					if (ta != 0)
					{
						dpix = NO_DEST_READ ? 0 : *dest;
						invta = 0x100 - ta;
						r = ((palbase[(pix >> 16) & 0xff] >> SRCSHIFT_R) * ta + DEST_R(dpix) * invta) >> 8;
						g = ((palbase[(pix >> 8) & 0xff] >> SRCSHIFT_G) * ta + DEST_G(dpix) * invta) >> 8;
						b = ((palbase[(pix >> 0) & 0xff] >> SRCSHIFT_B) * ta + DEST_B(dpix) * invta) >> 8;
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					...
					if (ta != 0)
					{
						dpix = NO_DEST_READ ? 0 : *dest;
						invsta = (0x10000 - ta) << 8;
						r = ((palbase[(pix >> 16) & 0xff] >> SRCSHIFT_R) * sr * ta + DEST_R(dpix) * invsta) >> 24;
						g = ((palbase[(pix >> 8) & 0xff] >> SRCSHIFT_G) * sg * ta + DEST_G(dpix) * invsta) >> 24;
						b = ((palbase[(pix >> 0) & 0xff] >> SRCSHIFT_B) * sb * ta + DEST_B(dpix) * invsta) >> 24;
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			if (palbase == NULL)
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = (SOURCE32_R(pix) * DEST_R(dpix)) >> (8 - SRCSHIFT_R);
					g = (SOURCE32_G(pix) * DEST_G(dpix)) >> (8 - SRCSHIFT_G);
					b = (SOURCE32_B(pix) * DEST_B(dpix)) >> (8 - SRCSHIFT_B);
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = (palbase[(pix >> 16) & 0xff] * DEST_R(dpix)) >> 8;
					g = (palbase[(pix >> 8) & 0xff] * DEST_G(dpix)) >> 8;
					b = (palbase[(pix >> 0) & 0xff] * DEST_B(dpix)) >> 8;
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			if (palbase == NULL)
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = (SOURCE32_R(pix) * sr * DEST_R(dpix)) >> (16 - SRCSHIFT_R);
					g = (SOURCE32_G(pix) * sg * DEST_G(dpix)) >> (16 - SRCSHIFT_G);
					b = (SOURCE32_B(pix) * sb * DEST_B(dpix)) >> (16 - SRCSHIFT_B);
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					pix = GET_TEXEL(argb32)(&prim->texture, curu, curv);
					dpix = NO_DEST_READ ? 0 : *dest;
					r = (palbase[(pix >> 16) & 0xff] * sr * DEST_R(dpix)) >> 16;
					g = (palbase[(pix >> 8) & 0xff] * sg * DEST_G(dpix)) >> 16;
					b = (palbase[(pix >> 0) & 0xff] * sb * DEST_B(dpix)) >> 16;
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			if (palbase == NULL)
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					...
					{
						dpix = NO_DEST_READ ? 0 : *dest;
						r = ((SOURCE32_R(pix) * ta) >> 8) + DEST_R(dpix);
						g = ((SOURCE32_G(pix) * ta) >> 8) + DEST_G(dpix);
						b = ((SOURCE32_B(pix) * ta) >> 8) + DEST_B(dpix);
						r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
						g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
						b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
						*dest = DEST_ASSEMBLE_RGB(r, g, b);
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					...
					{
						dpix = NO_DEST_READ ? 0 : *dest;
						r = ((palbase[(pix >> 16) & 0xff] * ta) >> (8 + SRCSHIFT_R)) + DEST_R(dpix);
						g = ((palbase[(pix >> 8) & 0xff] * ta) >> (8 + SRCSHIFT_G)) + DEST_G(dpix);
						b = ((palbase[(pix >> 0) & 0xff] * ta) >> (8 + SRCSHIFT_B)) + DEST_B(dpix);
						r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
						g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
						b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
						*dest = DEST_ASSEMBLE_RGB(r, g, b);
		...
		for (INT32 y = setup->starty; y < setup->endy; y++)
		{
			...
			if (palbase == NULL)
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					...
					{
						dpix = NO_DEST_READ ? 0 : *dest;
						r = ((SOURCE32_R(pix) * sr * ta) >> 24) + DEST_R(dpix);
						g = ((SOURCE32_G(pix) * sg * ta) >> 24) + DEST_G(dpix);
						b = ((SOURCE32_B(pix) * sb * ta) >> 24) + DEST_B(dpix);
						r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
						g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
						b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
						*dest = DEST_ASSEMBLE_RGB(r, g, b);
			...
			else
			{
				...
				for (INT32 x = setup->startx; x < endx; x++)
				{
					...
					{
						dpix = NO_DEST_READ ? 0 : *dest;
						r = ((palbase[(pix >> 16) & 0xff] * sr * ta) >> (24 + SRCSHIFT_R)) + DEST_R(dpix);
						g = ((palbase[(pix >> 8) & 0xff] * sr * ta) >> (24 + SRCSHIFT_R)) + DEST_G(dpix);
						b = ((palbase[(pix >> 0) & 0xff] * sr * ta) >> (24 + SRCSHIFT_R)) + DEST_B(dpix);
						r = (r | -(r >> (8 - SRCSHIFT_R))) & (0xff >> SRCSHIFT_R);
						g = (g | -(g >> (8 - SRCSHIFT_G))) & (0xff >> SRCSHIFT_G);
						b = (b | -(b >> (8 - SRCSHIFT_B))) & (0xff >> SRCSHIFT_B);
						*dest = DEST_ASSEMBLE_RGB(r, g, b);
...
static void FUNC_PREFIX(setup_and_draw_textured_quad)(const render_primitive *prim, void *dstdata, INT32 width, INT32 height, UINT32 pitch)
{
	...
	if (setup.startx < 0)
		setup.startx = 0;
	else if (setup.startx >= width)
		setup.startx = width;

	if (setup.endx < 0)
		setup.endx = 0;
	else if (setup.endx >= width)
		setup.endx = width;

	if (setup.starty < 0)
		setup.starty = 0;
	else if (setup.starty >= height)
		setup.starty = height;

	if (setup.endy < 0)
		setup.endy = 0;
	else if (setup.endy >= height)
		setup.endy = height;

	float gainwidth = 65536.0f * (float)prim->texture.width;
	float gainheight = 65536.0f * (float)prim->texture.height;
	/* compute start and delta U,V coordinates now */
	setup.dudx = round_nearest(gainwidth * fdudx);
	setup.dvdx = round_nearest(gainheight * fdvdx);
	setup.dudy = round_nearest(gainwidth * fdudy);
	setup.dvdy = round_nearest(gainheight * fdvdy);
	setup.startu = round_nearest(gainwidth * prim->texcoords.tl.u);
	setup.startv = round_nearest(gainheight * prim->texcoords.tl.v);
