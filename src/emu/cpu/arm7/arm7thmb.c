...
const void thumbblock01(arm_state *cpustate, UINT32 pc, UINT32 insn) /* Arithmetic */
{
	...
	/* Shift right */
		...
		else
		{
			...
			SET_REGISTER(cpustate, rd, (rrs & 0x80000000) ? ((0xFFFFFFFF << (32 - offs)) | (rrs >> offs)) : (rrs >> offs));
		}
		SET_CPSR(GET_CPSR & ~(N_MASK | Z_MASK));
		SET_CPSR(GET_CPSR | HandleALUNZFlags(GET_REGISTER(cpustate, rd)));
		R15 += 2;
				...
				case 0x4: /* ASR Rd, Rs */
					...
					if (rrs != 0)
					{
						...
						else
						{
							...
							SET_REGISTER(cpustate, rd, (rrd & 0x80000000) ? ((0xFFFFFFFF << (32 - rrs)) | (rrd >> rrs)) : (rrd >> rrs));
						}
					}
					SET_CPSR(GET_CPSR & ~(N_MASK | Z_MASK));
					SET_CPSR(GET_CPSR | HandleALUNZFlags(GET_REGISTER(cpustate, rd)));
...
const void thumbblock0c(arm_state *cpustate, UINT32 pc, UINT32 insn) /* Multiple Load/Store */
{
	...
	UINT32 rd = (insn & THUMB_MULTLS_BASE) >> THUMB_MULTLS_BASE_SHIFT;
	...
	UINT32 ld_st_address = GET_REGISTER(cpustate, rd);

	if (insn & THUMB_MULTLS) /* Load */
	{
		INT32 rd_in_list = insn & (1 << rd);
		for (offs = 0; offs < 8; offs++)
		{
			if (insn & (1 << offs))
			{
...
{
	INT32 offs = (INT8)(insn & THUMB_INSN_IMM);
