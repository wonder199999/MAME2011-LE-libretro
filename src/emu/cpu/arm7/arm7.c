...
{
	assert(device != NULL);
	assert( device->type() == ARM7 || device->type() == ARM7_BE ||
		device->type() == ARM7500 || device->type() == ARM9 ||
		device->type() == ARM920T || device->type() == PXA255);
...
void set_cpsr(arm_state *cpustate, UINT32 val)
{
	if (cpustate->archFlags & eARM_ARCHFLAGS_MODE26)
	{
		...
		else if (!(val & 0x10))	// mirror bits in pc
...
{
	UINT32 desc_lvl2 = (granularity == TLB_COARSE) ? ((first_desc & COPRO_TLB_CFLD_ADDR_MASK) | ((vaddr & COPRO_TLB_VADDR_CSLTI_MASK) >> COPRO_TLB_VADDR_CSLTI_MASK_SHIFT)) : vaddr;
	...
	switch (permission)
	{
		...
		case 1: // "Client - Accesses are checked against the access permission bits in the section or page descriptor"
		{
			switch (ap)
			{
				case 0:
				{
					if (COPRO_CTRL & COPRO_CTRL_SYSTEM)
					{
						if (!(COPRO_CTRL & COPRO_CTRL_ROM)) // "Only Supervisor read permitted"
						{
					...
					{
						if (!(COPRO_CTRL & COPRO_CTRL_ROM)) // "Any access generates a permission fault"
							return FAULT_PERMISSION;
...
int arm7_tlb_translate(arm_state *cpustate, UINT32 *addr, int flags)
{
	...
	UINT32 paddr, desc_lvl1, desc_lvl2 = 0;
	UINT32 vaddr = *addr;
	...
	if (vaddr < (32 * 1024 * 1024))
	{
		...
		if (pid > 0)
			vaddr += ((COPRO_FCSE_PID >> 25) & 0x7F) * (32 * 1024 * 1024);
	}
...
#endif
	domain = (desc_lvl1 >> 5) & 0x0f;
	permission = (COPRO_DOMAIN_ACCESS_CONTROL >> (domain << 1)) & 0x03;

	switch (desc_lvl1 & 0x03)
	{
		...
		{
			// Entry is a section
			UINT8 ap = (desc_lvl1 >> 10) & 0x03;
			INT32 fault = detect_fault(cpustate, permission, ap, flags);
			if (fault == FAULT_NONE)
	...
	if ((desc_lvl1 & 0x03) == COPRO_TLB_COARSE_TABLE || (desc_lvl1 & 0x03) == COPRO_TLB_FINE_TABLE)
	{
		switch (desc_lvl2 & 0x03)
		{
			case COPRO_TLB_UNMAPPED:
				// Unmapped, generate a translation fault
				if (flags & ARM7_TLB_ABORT_D)
			...
			{	// Small page descriptor
				UINT8 ap = ((((desc_lvl2 >> 4) & 0xFF) >> (((vaddr >> 10) & 0x03) << 1)) & 0x03);
				INT32 fault = detect_fault(cpustate, permission, ap, flags);
...
CPU_GET_INFO(arm7500)
{
	switch (state)
	{
		...
		default:			CPU_GET_INFO_CALL(arm7);		break;
	}
}
...
static READ32_DEVICE_HANDLER(arm7_rt_r_callback)
{
	...
	switch (cReg)
	{
					...
					switch (cpustate->archRev)
					{
						...
						case 4: // ARM7/SA11xx
							if (cpustate->archFlags & eARM_ARCHFLAGS_SA)
							{
								...
								// Stepping B5
								data = 0x69 | (0x01 << 16) | (0x0B11 << 4) | 0x09;
							}
							else
							{
								if (device->type() == ARM920T)
									data = (0x41 << 24) | (1 << 20) | (2 << 16) | (0x0920 << 4) | (0 << 0); // ARM920T (S3C24xx)
								else if (device->type() == ARM7500)
									data = (0x41 << 24) | (0 << 20) | (1 << 16) | (0x0710 << 4) | (0 << 0); // ARM7500
								else
		...
		case 5:             // Fault Status
			...
			if (op3 == 0)
				data = COPRO_FAULT_STATUS_D;
			else if (op3 == 1)
				data = COPRO_FAULT_STATUS_P;
		break;
	...
	switch (cReg)
 	{
		...
		break;
 		case 1:	// Control
			...
			LOG (("    MMU:%d, Address Fault:%d, Data Cache:%d, Write Buffer:%d\n", data & COPRO_CTRL_MMU_EN, (data & COPRO_CTRL_ADDRFAULT_EN) >> COPRO_CTRL_ADDRFAULT_EN_SHIFT,
				(data & COPRO_CTRL_DCACHE_EN) >> COPRO_CTRL_DCACHE_EN_SHIFT, (data & COPRO_CTRL_WRITEBUF_EN) >> COPRO_CTRL_WRITEBUF_EN_SHIFT));
			LOG (("    Endianness:%d, System:%d, ROM:%d, Instruction Cache:%d\n", (data & COPRO_CTRL_ENDIAN) >> COPRO_CTRL_ENDIAN_SHIFT,
				(data & COPRO_CTRL_SYSTEM) >> COPRO_CTRL_SYSTEM_SHIFT, (data & COPRO_CTRL_ROM) >> COPRO_CTRL_ROM_SHIFT, (data & COPRO_CTRL_ICACHE_EN) >> COPRO_CTRL_ICACHE_EN_SHIFT));
		...
		case 5:             // Fault Status
			...
			if (op3 == 0)
				COPRO_FAULT_STATUS_D = data;
			else if (op3 ==1)
				COPRO_FAULT_STATUS_P = data;
		break;
		case 6:             // Fault Address
