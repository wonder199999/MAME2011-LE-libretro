...
static void display_rom_load_results(rom_load_data *romdata)
{
	...
	if (romdata->errors != 0)
	{
		...
		fatalerror_exitcode(romdata->machine, MAMERR_MISSING_FILES, "ERROR: required files are missing, the machine cannot be run.");
	}

	/* if we had warnings, output them, but continue */
	if (romdata->warnings)
	{
		romdata->errorstring.cat("WARNING: the machine might not run correctly.");
